...
class Emitter():
    ...
    *   @param in the type of the local array variable.
    '''
    def emitLOCALARRAY(self, index, in_, frame):
        result = list()
        result.append(self.emitPUSHICONST(in_.dimen, frame))
        if isinstance(in_.eleType, StringType): 
           result.append(self.jvm.emitANEWARRAY(self.getFullType(in_.eleType)))
        else:
            result.append(self.jvm.emitNEWARRAY(self.getFullType(in_.eleType)))
        frame.pop()
        result.append(self.jvm.emitASTORE(index))
        return ''.join(result)
    ...
    *   @param in the list of symbol entries corresponding to local array variable.    
    '''
    def emitSTATICARRAY(self, lexeme, in_, frame):
        result = list()
        result.append(self.emitPUSHICONST(in_.dimen, frame))
        if isinstance(in_.eleType, StringType): 
           result.append(self.jvm.emitANEWARRAY(self.getFullType(in_.eleType)))
            ...
            result.append(self.jvm.emitNEWARRAY(self.getFullType(in_.eleType)))
        result.append(self.emitPUTSTATIC(lexeme, in_, frame))
        return ''.join(result)
